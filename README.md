...
- **זמני שבת/חג** — נשאבים מאתר [ישיבה](https://www.yeshiva.org.il) (HTML), עבור ירושלים ואפרת.
  - **מקור גיבוי:** אם אתר ישיבה אינו זמין (ב‑10–11.9.2026 הוא עבר מאחורי Cloudflare
    והחזיר `403`, מה שהפיל את הבוט) — הבוט נופל אוטומטית ל‑Hebcal.
    ההודעה במצב זה מסומנת בבאנר אזהרה מפורש, ו**זמן רבינו תם אינו זמין בה**.
  - היסטי הדלקת הנרות במצב הגיבוי מוגדרים בקבועים `CANDLE_MIN_JM` (40 דק׳, מנהג
    ירושלים) ו‑`CANDLE_MIN_EF` (20 דק׳) שבראש `src/code-node.js`, ובפרמטר `b=`
    שב‑URL של נודי ה‑Hebcal. ⚠️ ייתכנו הפרשי דקות מזמני אתר ישיבה.
...
- **`buildShabbatCandidatesFromHebcal`** — אותו דבר מ‑Hebcal, כמקור גיבוי. מזהה בלוק
  רציף (למשל שבת שנכנסת לראש השנה) לפי רצף `candles` ללא `havdalah` ביניהם.
...
- **`buildResults`** — בוחר את האירוע/ים שצריך להתריע עליהם *היום* (לקראת מחר),
  ומחליט באיזה מקור זמנים להשתמש. אם **שני** המקורות נפלו הוא מייצר התראת תקלה
  (`kind: 'alert'`) במקום להיכשל בשקט.
