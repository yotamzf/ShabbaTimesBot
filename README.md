...
│   + (Next Year)      │   │     + (Next Year)     │     (ר״ת בלבד; שתי שנים
└──────────┬───────────┘   └──────────┬───────────┘      עבריות, ממוזגות)
           ▼                          ▼
┌──────────────────────┐   ┌──────────────────────┐
...
└──────────┬───────────┘   └──────────┬───────────┘     (צומות + ראש חודש, JSON)
           ▼
┌──────────────────────┐   ┌──────────────────────┐
│ Fetch JM Shabbat      │   │  Fetch Efrat Shabbat  │  🌐 hebcal.com API
│      (Hebcal)         │   │       (Hebcal)        │     (מקור גיבוי לזמני שבת/חג)
└──────────┬───────────┘   └──────────┬───────────┘
           ▼
┌──────────────────────┐
...
└──────────┬───────────┘
           ...
           ▼ (true)                   ▼ (true)
┌──────────────────────┐   ┌──────────────────────┐
│   Send Shabbat Email  │   │  Send Shabbat Telegram│  📧 + 💬 (ענפים מקבילים)
└──────────────────────┘   └──────────────────────┘
```

### מקורות הנתונים
- **זמני שבת/חג** — מ‑[Hebcal REST API](https://www.hebcal.com/home/195/jewish-calendar-rest-api)
  (JSON), עם `ue=on` כדי שהחישוב יתחשב בגובה מעל פני הים.
- **זמן רבינו תם** — מאתר [ישיבה](https://www.yeshiva.org.il), להעשרה בלבד. Hebcal
  אינו מספק אותו בנקודת הקצה הזו; אם האתר לא זמין, שורת ר״ת פשוט מושמטת.
  היסטי ההדלקה מוגדרים בקבועים `CANDLE_MIN_JM` (40 דק׳, מנהג ירושלים) ו‑
  `CANDLE_MIN_EF` (20 דק׳, אומת מול אתר ישיבה) שבראש `src/code-node.js`,
  ובפרמטר `b=` שב‑URL של נודי ה‑Hebcal. שינוי באחד מחייב שינוי בשני.
- **צומות, ראש חודש וימים לאומיים** — מ‑Hebcal.

> **למה Hebcal ראשי ולא אתר ישיבה?** נבדק מול נתונים חיים ב‑11.9.2026:
> 1. מערך `times` של אתר ישיבה מכסה **שנה עברית אחת בלבד**. באותו יום השורה
>    האחרונה הייתה `5/9/2026` — ולכן בערב ראש השנה הבוט לא זיהה אירוע ושתק,
>    בלי שגיאה ובלי הודעה. תקלה שחוזרת מדי שנה, בדיוק בערב ראש השנה.
>    הפרמטר `year=` פותר, ולכן מושכים שתי שנים עבריות (נוכחית + הבאה) וממזגים.
> 2. ב‑10–11.9.2026 האתר החזיר `403` מאחורי Cloudflare. הסיבה: נודי ה‑HTTP
>    שלחו `User-Agent` שמתחזה ל‑Chrome, ו‑Cloudflare הגיש להם אתגר. בקשות
>    **בלי** הכותרת המזויפת נענו כרגיל. הכותרת הוסרה — גם יותר ישר וגם יותר
>    אמין. עדיין עדיף לא להישען על scraping כמקור ראשי.
>
> **דיוק:** עם `ue=on` ההפרש בין המקורות נמדד כדקה אחת לכל היותר
> (שבת ראש השנה תשפ״ז: ירושלים 18:14 מול 18:15, אפרת 18:35 מול 18:34).
...
### עמידות בתקלות
- ⚠️ **חובה לפרסם (Publish) אחרי כל שינוי ב‑n8n.** עריכה שומרת טיוטה בלבד;
  ריצות production — כולל ה‑Schedule Trigger היומי — מריצות את הגרסה
  המפורסמת. שינוי שלא פורסם פשוט לא ירוץ ב‑08:00.
- כל נודי ה‑HTTP מוגדרים `retryOnFail` (3 ניסיונות, 5 שניות ביניהם) עם
  `onError: continueRegularOutput` — מקור שנופל מעביר הלאה פריט שגיאה במקום
  להפיל את כל הריצה. `safeNode` בקוד מזהה זאת ומחזיר `null`.
- **מייל וטלגרם הם שני ענפים מקבילים** של `Should Notify?`. קודם הם היו משורשרים,
  כך שכשל ב‑Gmail היה מונע גם את הודעת הטלגרם.
...
- **`buildShabbatCandidatesFromHebcal`** — המקור הראשי. מזהה בלוק רציף (למשל שבת
  שנכנסת לראש השנה) לפי רצף `candles` ללא `havdalah` ביניהם.
- **`mergeYeshivaTimes`** — ממזג את שתי השנים העבריות שנמשכות מאתר ישיבה.
- **`enrichFromYeshiva`** — מוסיף זמן ר״ת ומצליב את שאר הזמנים. פער גדול מ‑
  `CROSSCHECK_TOLERANCE_MIN` (3 דק׳) מצורף כאזהרה להודעה במקום להיבלע.
...
- **`buildResults`** — בוחר את האירוע/ים שצריך להתריע עליהם *היום* (לקראת מחר).
  **מצליב בין המקורות:** אם מקור אחד מזהה אירוע להיום והשני שותק — הולך אחרי
  זה שיש לו נתונים ומסמן זאת בהודעה. אם **שניהם** שותקים לגמרי הוא מייצר
  התראת תקלה (`kind: 'alert'`) במקום להיכשל בשקט.
